...
class PR_Task: public Task {
    ...
    protected:
		...
		void pointGeomDiff(int trNum, unsigned long i, unsigned long k, const Vector &PiEtAx, TNetF &c, TNetF &x_Net) const;
        unsigned long storeResults(int trNum, unsigned long j, unsigned long m, unsigned long k, VecOfVec &vx_i, VecOfVec &vu_i,
							   VecOfVec &vv_i);
